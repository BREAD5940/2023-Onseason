...
/** Add your docs here. */
public class BalenceTrajectoryCommand extends TrajectoryFollowerCommand {

	public BalenceTrajectoryCommand(PathPlannerTrajectory trajectory, Supplier<Rotation2d> startHeading, Swerve swerve, boolean stop) {
		super(trajectory, startHeading, swerve, stop);
	}

	public BalenceTrajectoryCommand(PathPlannerTrajectory trajectory, Swerve swerve, boolean stop) {
	...
	public void execute() {
		...
		if (currentRobotPose == trajectory.getEndState().poseMeters){
			wpilibGoal.poseMeters = new Pose2d(wpilibGoal.poseMeters.getX(), currentRobotPose.getY(), wpilibGoal.poseMeters.getRotation());
		}
